...
|:---|:---|:---|:---:|:---:|
| HU-01 | Como **cajero**, quiero registrar un cliente natural con sus datos personales para que pueda acceder a los servicios del banco | - El sistema valida que el documento de identidad no esté registrado previamente<br>- Todos los campos obligatorios deben ser diligenciados<br>- Se muestra un mensaje si el banco alcanza el límite de clientes<br>- El cliente queda registrado correctamente en el sistema | Alta | 5 |
| HU-02 | Como **cajero**, quiero registrar un cliente empresarial con sus datos legales para que pueda acceder a los servicios del banco | - El sistema valida que el NIT no esté registrado previamente<br>- Todos los campos obligatorios deben ser diligenciados<br>- Se registran los datos de la empresa correctamente<br>- El cliente empresarial queda almacenado en el sistema | Alta | 5 |
| HU-03 | Como **cajero**, quiero abrir una cuenta bancaria para un cliente registrado para que pueda realizar transacciones | - El sistema valida que el cliente exista<br>- Un cliente no puede tener más de 5 cuentas<br>- Se debe seleccionar un tipo de cuenta válido<br>- La cuenta se crea correctamente y queda asociada al cliente | Alta | 5 |
| HU-04 | Como **cajero**, quiero realizar depósitos para aumentar el saldo de una cuenta | - El valor a depositar debe ser mayor a cero<br>- La cuenta debe estar activa<br>- El saldo se actualiza correctamente<br>- La transacción queda registrada | Alta | 4 |
| HU-05 | Como **cajero**, quiero realizar retiros para que el cliente pueda disponer de su dinero | - La cuenta debe tener saldo suficiente<br>- La cuenta debe estar activa<br>- El saldo se actualiza correctamente<br>- La transacción queda registrada | Alta | 5 |
| HU-06 | Como **asesor financiero**, quiero realizar transferencias entre cuentas para mover dinero entre clientes | - Ambas cuentas deben existir<br>- Ambas cuentas deben estar activas<br>- La cuenta origen debe tener saldo suficiente<br>- La transferencia se registra correctamente | Alta | 5 |
| HU-07 | Como **asesor financiero**, quiero consultar información de clientes para brindar asesoría | - El sistema permite buscar clientes por su identificación<br>- Se muestra la información del cliente si existe<br>- Se informa si el cliente no está registrado | Media | 3 |
| HU-08 | Como **asesor financiero**, quiero bloquear y desbloquear cuentas según criterios de seguridad para proteger los fondos del cliente | - La cuenta debe existir<br>- El estado de la cuenta cambia correctamente<br>- Se registra el cambio de estado | Media | 3 |
| HU-09 | Como **gerente de sucursal**, quiero aprobar créditos para autorizar préstamos a clientes | - Solo el gerente puede aprobar créditos<br>- Se evalúan las condiciones del cliente<br>- El estado del crédito cambia a aprobado cuando cumple los requisitos | Alta | 5 |
| HU-10 | Como **gerente de sucursal**, quiero registrar empleados para gestionar el personal del banco | - Todos los datos obligatorios deben ser diligenciados<br>- No se permiten empleados duplicados<br>- El empleado queda registrado correctamente | Media | 3 |
| HU-11 | Como **gerente de sucursal**, quiero calcular la nómina para pagar salarios de los empleados | - El salario incluye bonificaciones según desempeño<br>- Se tiene en cuenta la antigüedad del empleado<br>- El cálculo del salario es correcto para cada tipo de empleado | Media | 3 |
| HU-12 | Como **cliente**, quiero recibir notificaciones para estar informado de mis transacciones | - El cliente recibe una notificación por cada transacción<br>- La información enviada es clara y comprensible | Baja | 2 |
| HU-13 | Como **sistema auditor**, quiero consultar el historial de transacciones para llevar control interno | - Se muestran las transacciones realizadas en cada cuenta<br>- El historial incluye fecha y usuario asociado<br>- Se respeta el límite de registros por cuenta | Media | 4 |
